...
class Item(AbstractPage):
	if item_settings.MULTIPLE_CATEGORY:
		categories   = models.ManyToManyField(verbose_name=_("Категорія"), to='sw_item.ItemCategory', related_name="items", blank=True)    
	else:
		category     = TreeForeignKey(verbose_name=_("Категорія"), to='sw_item.ItemCategory', related_name="items", on_delete=models.SET_NULL, blank=True, null=True)    
	markers      = models.ManyToManyField(verbose_name=_("Маркери"), to='sw_item.ItemMarker', related_name='items', blank=True)
	similars     = models.ManyToManyField(verbose_name=_("Супутні товари"), to="self", related_name="similars_set", blank=True, default=None)
	manufacturer = models.ForeignKey(verbose_name=_("Виробник"), to="sw_item.ItemManufacturer", blank=True, null=True, on_delete=models.SET_NULL, related_name='items')
	brand        = models.ForeignKey(verbose_name=_("Бренд"), to='sw_item.ItemBrand', related_name='items', on_delete=models.SET_NULL, null=True, blank=True)
	in_stock     = models.ForeignKey(verbose_name=_("Наявність"), to="sw_item.ItemStock", on_delete=models.SET_NULL, blank=True, null=True, help_text='"Кількість" в пріорітеті над "наявністю"')
	currency     = models.ForeignKey(verbose_name=_("Валюта"),    to="sw_item.ItemCurrency",     related_name="items", on_delete=models.SET_NULL, help_text=("Якщо залишити порожнім, то буде встановлена валюта категорії, у якій знаходиться товар"), blank=True, null=True)
